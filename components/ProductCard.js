    ...
    <div className="bg-white rounded-xl shadow-sm hover:shadow-md border border-gray-100 overflow-hidden flex flex-col group transition-all duration-300">
      <Link href={`/product/${product.slug}`} className="relative block overflow-hidden aspect-square bg-gray-50">
        <img
          src={imageUrl}
          alt={product.name}
          className="object-contain w-full h-full p-3 md:p-4 group-hover:scale-105 transition-transform duration-300"
        />
        {hasDiscount && (
          <span className="absolute top-2 left-2 bg-brandOrange text-brandBlue text-[10px] md:text-xs font-bold px-2 py-1 rounded-md shadow-sm">
      ...
      <div className="p-3 md:p-4 flex flex-col flex-grow space-y-1.5">
        ...
        <Link href={`/product/${product.slug}`} className="font-bold text-brandBlue hover:text-brandOrange line-clamp-2 text-xs md:text-base leading-tight h-8 md:h-10">
          {product.name}
        </Link>
        ...
        <div className="flex items-center space-x-1 py-0.5">
          {[1, 2, 3, 4, 5].map((star) => (
            <span key={tab.id} className="text-amber-400 font-bold">★</span>
          ))}
        ...
        <button
          ...
          className={`w-full mt-2 py-2 md:py-2.5 rounded-lg font-bold text-xs flex items-center justify-center gap-1.5 transition-all ${
            product.stock > 0 
